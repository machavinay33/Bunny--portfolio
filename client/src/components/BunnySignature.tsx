...
export const BUNNY_SEQUENCE_MS = 3250;
export const BUNNY_REPLAY_GAP_MS = 4000;
...
export default function BunnySignature() {
  ...
  useEffect(() => {
    ...
    const runCycle = () => {
      setPhase("intro");
      later(() => setPhase("arrived"), 1550);
      later(() => setPhase("dissolve"), 1750);
      later(() => {
        setPhase("complete");
        later(() => {
  ...
  return <div className={`bunny-stage ${completed ? "is-complete" : ""}`} aria-label="Bunny logo signature animation">
    {!completed && <div key={`rabbit-${cycle}`} className={`rabbit-rig ${phase === "arrived" ? "arrived" : ""} ${phase === "dissolve" ? "dissolving" : ""}`}>
    ...
    {!completed && <div key={`particles-${cycle}`} className="bunny-particles">{particles.map(p => <i key={p.id} style={{ "--x0": `${p.x}px`, "--y0": `${p.y}px`, "--xt": `${p.tx}px`, "--yt": `${p.ty}px`, "--s": `${p.size}px`, "--d": `${p.delay}s` } as React.CSSProperties} />)}</div>}
    <div key={`mark-${cycle}`} className="bunny-mark reveal"><Mark /></div>
    <div key={`wordmark-${cycle}`} className="bunny-wordmark reveal">{["B", "U", "N", "N", "Y"].map((letter, i) => <span key={`${letter}-${i}`} style={{ animationDelay: `${2.55 + i * .1}s` }}>{letter}</span>)}</div>
    <div key={`tagline-${cycle}`} className="bunny-tagline reveal">BUILD&nbsp;•&nbsp;CREATE&nbsp;•&nbsp;INSPIRE</div>
  </div>;
}
